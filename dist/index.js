...
var rateLimitMiddleware = /* @__PURE__ */ __name(async (c, next) => {
  ...
  if (existing) {
    ...
    const expiresAt = new Date(claimedAt.getTime() + TTL_SECONDS * 1e3);
    return c.json(
      {
...
async function recordSuccessfulRequest(kv, ip) {
  await kv.put(`${KV_PREFIX}${ip}`, (/* @__PURE__ */ new Date()).toISOString(), {
    expirationTtl: TTL_SECONDS
  });
}
__name(recordSuccessfulRequest, "recordSuccessfulRequest");

// src/fireblocks.ts
var FIREBLOCKS_API_BASE = "https://api.fireblocks.io";
async function sendFaucetTransaction(env2, assetId, destinationAddress, amount) {
  ...
  const token = await signJwt(env2.FIREBLOCKS_API_KEY, env2.FIREBLOCKS_SECRET_KEY, path, body);
  const res = await fetch(`${FIREBLOCKS_API_BASE}${path}`, {
    method: "POST",
    headers: {
      "Content-Type": "application/json",
      "X-API-Key": env2.FIREBLOCKS_API_KEY,
...
__name(sendFaucetTransaction, "sendFaucetTransaction");
async function signJwt(apiKey, secretKeyPem, uri, body) {
...
__name(base64urlFromBuffer, "base64urlFromBuffer");
